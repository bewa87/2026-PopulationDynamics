...
start_ind   = find(world_year==1993);
...
for j = 1:1:n
  ...
  hold on
  title('Example 3 - Dependence of k on P (Cubic spline)','fontsize',14);
...
hold on
title('Example 3 - Total German Population (Cubic spline)','fontsize',14);
legend({'Data used','Calculation'},'location','northwest','fontsize',10);
xlabel('Time (Year)','fontsize',10);
ylabel('Total German Population','fontsize',10);
xticks([0:4*time_int:28]);
xticklabels({'1994','1998','2002','2006','2010','2014','2018','2022'});
...
figure(4)
plot([0:time_int:22]',err_rel,'marker','*','color','blue','linewidth',1);
hold on
title('Example 3 - Relative Error (Cubic spline)','fontsize',14);
xlabel('Time (Year)','fontsize',10);
...
% Step 7: Coefficient in front of cubic against time

third_coeff      = diff(M)/6;
